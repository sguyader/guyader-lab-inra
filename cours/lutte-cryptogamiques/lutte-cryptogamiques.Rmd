...
DiagrammeR::grViz("eumycetes2.dot")
...
- .Large[Reproduction asexuée (phase anamorphe) :]<br><br>
  ...
  * .large[assure la propagation et la multiplication de la population à courte distance]
  * .large[se réalise soit par fragmentation du thalle, soit par sporulation]
--
<br><br>
- .Large[Reproduction sexuée (phase téléomorphe ou forme parfaite) :]
...
.pull-left[
.Large[
- Sous la forme de structures de résistances (sclérotes, chlamydospores, oospores)
 ...
 * la dispersion à de nouveau hôtes
<br>
...
### Comment mesurer le développement d'une maladie
...
- **Une alternative** : compter le nombre de spores présentes dans l’air ambiant, à la surface des organes, dans le sol
]
...
.Large[
- Monocyclique
 Développement de la maladie correspondant à un **seul cycle de développement** du parasite pendant _un cycle cultural_
.center[
```{r echo=F, out.width="40%"}
...
- Polycyclique
 Développement de la maladie correspondant à une juxtaposition de **multiples monocycles** pendant _un cycle cultural_
]
 ...
 * quantité de tisus ou de plantes sensibles
...
.Large[
- ex. : bouillie bordelaise, fumigation et désifection des sols
- peu sélectifs
...
- Essentillement utilsés à titre préventif, rarement curatif
]
...
.blue-block[
.Large[Réduction de l’inoculum ou de l’activité d’un bioagresseur par au moins un **organisme non-humain** (_lutte biologique_) ou par des **produits issus du vivant** (_biocontrôle_)
]
]
<br>

.Large[
- Utilisation de sols suppressifs : favorisent le maintien et l’**activité d’organismes antagonistes** (bactéries *Pseudomonas fluorescens*…, champignons *Trichoderma* spp. …)
<br><br>
- Application directe d’antagonistes (unique ou répétée) :
 * protection croisée (**stimulation des défenses** naturelles de la plante)
 * pré-colonisation des sites (**compétition** interspécifique)
 * mycoparasitisme (champignons parasités par d'autres microbes)
 * prédation (organismes consommant les champignons pathogènes)
 * production de métabolites antifongiques
]
...
.center[
```{r echo=F, out.width="50%"}
knitr::include_graphics("./images/biocontrole1.jpg")
```
]
...
.Large[Possibilités variées, en focntion des pathosystèmes :]
.large[
* éviter un sol infecté (rotation)
* utilisation d’une plante assainissante en interculture (piège, allélopathie)
...
<br><br><br>
&rarr; Utilisation de modèles de nuisibilité et d’économie
]
...
.Large[
.indent[.indent[**SPE** = seuil de pertes économiques]]
<br>
.indent[.indent[**SA** = seuil d'action]]
]
