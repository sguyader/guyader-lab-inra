...
postscript("fig1.eps", width=7, height=7, horizontal=FALSE, onefile=FALSE, paper="special", family="Helvetica")
xyplot(ln.sporedensity ~ distance|runlevel, data=disper2.g, xlab="Distance (cm)", ylab=list(expression(bold(paste("ln(densité de spores par ",cm^2,")")))),
  panel=function(x,y){
   panel.xyplot(x,y)
...
anova(m.lin2,m.lin3)  # m.lin3 est meilleur
...
plot1 <- xyplot(mean.ln.spod ~ distance, groups=factor(run, levels=c(2,3,4,5,6,7,8,10)), data=tab.moy.spod, type="b", xlim=c(8,42), ylim=c(-1,8), main="",
                xlab=list("Distance (cm)"), ylab=list(expression(bold(paste("ln(densité de spores par ",cm^2,")")))), 
...
plot2 <- xyplot(ln.sporedensity~ distance, data=disper2.g, xlim=c(8,42), ylim=c(-1,8), xlab=list("Distance (cm)"), ylab=list(expression(bold(paste("ln(densité de spores par ",cm^2,")")))), panel=function(x,y,...)
{
  panel.xyplot(x,y,...)
...
postscript("fig2.eps", width=7, height=7, horizontal=FALSE, onefile=FALSE, paper="special", family="Helvetica")
c(plot2,plot1, layout=c(1,2), y.same=F, merge.legends=T)
dev.off()

postscript("fig3.eps", width=4.5, height=3, horizontal=FALSE, onefile=FALSE, paper="special", family="Helvetica")
plot(m.lin3, xlab="Valeurs ajustées", ylab="Résidus standardisés",grid=F)
dev.off()

# Fitted vs Observed
graph.A <- xyplot(disper2.g$ln.sporedensity[!is.na(disper2.g$ln.sporedensity)]~fitted(m.lin3), lty=1, xlab=list("Valeurs ajustées"), ylab=list("Valeurs observées"),
  panel= function(x,y) {
...
graph.B <- xyplot(sort(resid)~sort(theor), xlim=c(-3,3), ylim=c(-4,4), xlab=list("Quantiles théoriques (loi normale)"), ylab=list("Résidus standardisés"),
  panel= function(x,y) {
    ...
    #panel.points(theor,low.band,type="l",lty=1)
    #panel.points(theor,high.band,type="l",lty=1)
    panel.xyplot(x,y)
    panel.qqmathline(x,y)
  })

# Combine graphs A and B on same page
postscript("fig4.eps", width=7, height=7, horizontal=FALSE, onefile=FALSE, paper="special", family="Helvetica")
...
postscript("fig5.eps", width=4, height=4, horizontal=FALSE, onefile=FALSE, paper="special", family="Helvetica")
...
postscript("fig6.eps", width=4, height=4, horizontal=FALSE, onefile=FALSE, paper="special", family="Helvetica")
xyplot(cbind(mean.ln.spod,lower,upper)~distance, data=tab.moy.spod2, xlim=c(0,42), xlab="Distance (cm)", ylab=list(expression(bold(paste("ln(densité de spores par ",cm^2,")")))), panel=panel.errbars, ewidth=1, make.grid="none")
trellis.focus()
panel.curve(expr=params[1]+params[2]*x)
trellis.unfocus()
dev.off()

postscript("fig7.eps", width=4, height=4, horizontal=FALSE, onefile=FALSE, paper="special", family="Helvetica")
