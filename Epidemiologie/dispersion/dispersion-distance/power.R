...
## Modèle Pwer
postscript("fig1-power.eps", width=7, height=7, horizontal=FALSE, onefile=FALSE, paper="special", family="Helvetica")
...
m.power1 <- lm(ln.sporedensity~ln.distance, data=disper2.g)
summary(m.power1)

m.power2 <- lme(fixed=ln.sporedensity~1, random=~1|run, data=disper2.g, method="ML") # unconditional means model
summary(m.power2)
VarCorr(m.power2)
# run = pdLogChol(1) 
#             Variance  StdDev   
...
tau.sq <- as.numeric(VarCorr(m.power2)[1,1])
sigma.sq <- as.numeric(VarCorr(m.power2)[2,1])
tau.sq/(tau.sq+sigma.sq)
# [1] 2.145653e-09      # la corrélation entre observations d'un même run est nulle

m.power3 <- lme(fixed=ln.sporedensity~1+ln.distance, random=~1|run, data=disper2.g, method="ML") # random intercept model
(as.numeric(VarCorr(m.power2)[2,1]) - as.numeric(VarCorr(m.power3)[2,1]))/ as.numeric(VarCorr(m.power2)[2,1]) # Pseudo-R2 pour influence de distance sur niveau 1
# [1] 0.7834556 = Pseudo R2

anova(m.power2,m.power3)  # m.power3 est meilleur

#m.power4 <- lme(fixed=ln.sporedensity~1+log(distance), random=~1+distance|run, data=disper2.g, method="ML") # random intercept model
#anova(m.power3,m.power4)  # pas d'amélioration AIC, m.power3 préféré par BIC

BIC(m.power1, m.power3) # m.power3 préféré à m.power1

subj.specific.parms<-coef(m.power3)
pop.average.parms<-fixef(m.power3)
...
postscript("fig2-power.eps", width=7, height=7, horizontal=FALSE, onefile=FALSE, paper="special", family="Helvetica")
c(plot2,plot1, layout=c(1,2), y.same=F, merge.legends=T)
dev.off()

postscript("fig3-power.eps", width=4.5, height=3, horizontal=FALSE, onefile=FALSE, paper="special", family="Helvetica")
plot(m.power3, xlab="Valeurs ajustées", ylab="Résidus standardisés",grid=F)
dev.off()

# Fitted vs Observed
graph.A <- xyplot(disper2.g$ln.sporedensity[!is.na(disper2.g$ln.sporedensity)]~fitted(m.power3), lty=1, xlab=list("Valeurs ajustées"), ylab=list("Valeurs observées"),
                  panel= function(x,y) {
...
# QQPlot
require(envelope)
resid <- residuals(m.power3,type="pearson")
...
postscript("fig4-power.eps", width=4, height=7, horizontal=FALSE, onefile=FALSE, paper="special", family="Helvetica")
print(graph.A, pos = c(0.0, 0.5, 1.0, 1.0), more = T)
print(graph.B, pos = c(0.0, 0.0, 1.0, 0.5), more = F)
dev.off()

# Calcul de la d50
params <- fixef(m.power3)
params
# (Intercept)    distance 
# 8.0020788  -0.1564117 
...
postscript("fig5-power.eps", width=4, height=4, horizontal=FALSE, onefile=FALSE, paper="special", family="Helvetica")
...
postscript("fig6-power.eps", width=4, height=4, horizontal=FALSE, onefile=FALSE, paper="special", family="Helvetica")
...
postscript("fig7-power.eps", width=4, height=4, horizontal=FALSE, onefile=FALSE, paper="special", family="Helvetica")
