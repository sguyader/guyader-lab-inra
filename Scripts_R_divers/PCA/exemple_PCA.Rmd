...
pcaplot <- res.PCA %>% fviz_pca_biplot(repel=T, axes=c(1,2), title="", col.var="Black",
                            col.ind=data_CG_host2$Colleto, palette="RdBu")
pcaplot


pcaplot <- res.PCA %>% fviz_pca_biplot(repel=T, axes=c(1,2), title="",
                            geom.ind=c("text"))

pcaplot +
  geom_point(aes(x=res.PCA$ind$coord[,1], y=res.PCA$ind$coord[,2],
                 col=data_CG_host2$Colleto, shape=data_CG_host2$Colleto)) +
  theme_bw()
...
i=0
while(i < ncol(CG.host_modif)){
  i=i+1
  CG.host_modif[,i] <- as.factor(CG.host_modif[,i])
}



res.FAMD <- FAMD(CG.host_modif[,1:21])
res.FAMD %>% plot.FAMD(choix="ind")
res.FAMD %>% plot.FAMD(choix="var")

res.FAMD %>% fviz_famd(axes=c(1,2), title="") +
  theme_bw()
