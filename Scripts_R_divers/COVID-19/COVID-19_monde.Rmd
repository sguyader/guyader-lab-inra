...
list.of.packages <- c("tidyverse","rnaturalearth","sf","minpack.lm","kableExtra")
new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
if(length(new.packages)) install.packages(new.packages)
for (i in unique(list.of.packages)) {library(i, character.only = TRUE)}
...
Boucle pour ajuster le modèle à chaque région/pays :
...
for(i in unique(jhu.covid$geounit)) {
  tryCatch({ # fonction pour intercepter les erreurs et laisser la boucle de continuer
    ...
    model1 <- nlsLM(gomp.fn, data=dat, start=list(b=0.1, Asym=1e6, xmid=1.83e4))  # ajustement nonliéaire avec algorithme Levenberg-Marquardt
...
Cas particulier de l'Australie, le Cananda et la Chine : les données sont uniquement par région il faut donc faire la somme des régions pour avoir la dynamique au niveau pays
