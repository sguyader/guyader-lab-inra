...
data_covid_ts <- download_merged_data(cached = TRUE, silent = TRUE) %>%
...
data_covid_ts <- data_covid_ts %>%
  ...
  #                                                  confirmed_per_million, method="linear", na.rm=T), .after=confirmed_per_million) %>%
  mutate(deaths_per_million = deaths/population*1e6, .after=deaths) %>%
  #mutate(auc_deaths_per_million = DescTools::AUC(as.numeric(date),
  #                                               deaths_per_million, method="linear", na.rm=T), .after=deaths_per_million) %>%
  ...
  #                                                  recovered_per_million, method="linear", na.rm=T), .after=recovered_per_million) %>%
  ...
  #                                                     active_cases_per_million, method="linear", na.rm=T), .after=active_cases_per_million) %>%
  ...
  mutate(hosp_per_million = hosp_patients/population*1e6, .after=hosp_patients) %>%
  #mutate(auc_hosp_per_million = DescTools::AUC(as.numeric(date),
  #                                             hosp_per_million, method="linear", na.rm=T), .after=hosp_per_million) %>%
  mutate(icu_per_million = icu_patients/population*1e6, .after=icu_patients) %>%
