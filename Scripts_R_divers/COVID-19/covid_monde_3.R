...
results <- tibble(params=c("b","Asym","xmid"))
...
for(i in unique(jhu.covid$geounit)) {
  tryCatch({ # fonction pour intercepter les erreurs et laisser la boucle de continuer
    dat <- filter(jhu.covid, geounit == i) # filtrer par région/pays
    model1 <- nlsLM(gomp.fn, data=dat, start=list(b=0.1, Asym=1.7e4, xmid=1.83e4))  # ajustement nonliéaire avec algorithme Levenberg-Marquardt
...
australia <- filter(jhu.covid, Country.Region == "Australia") %>% group_by(date) %>% mutate(confirmed = sum(confirmed))
canada <- filter(jhu.covid, Country.Region == "Canada") %>% group_by(date) %>% mutate(confirmed = sum(confirmed))
china <- filter(jhu.covid, Country.Region == "China") %>% group_by(date) %>% mutate(confirmed = sum(confirmed))

AusCanChi <- bind_rows(australia,canada,china)

# Boucle pour l'analyse
for(i in unique(AusCanChi$Country.Region)) {
  tryCatch({ # fonction qui intercepte les messages d'erreur et laisse la boucle continuer
    dat <- filter(AusCanChi, Country.Region == i)
    model1 <- nlsLM(gomp.fn, data=dat, start=list(b=0.1, Asym=1.4e5, xmid=1.83e4))
    res <- tibble(coef(model1))
    names(res) <- i
    ...
    p <- ggplot(dat) +
      ...
      theme(axis.text.x = element_text(angle = 90)) +
      labs(title = paste("Evolution des cas officiels de COVID-19 en", i), x="date", y="nombre de cas confirmés")
    ggsave(p, filename=paste("plot_",i,".png"), path="dynplots", width=8, height=5, units="in", dpi=72)
  }, error=function(e){cat("ERROR :",i, "\n")}) # affiche le nom du pays/région qui pose problème
}

# Tableau de résultats des ajustements
print(results)
write.csv(results, "gompertz_param_table.csv")

results.long <- results %>% pivot_longer(-params, names_to = "Country.Region", values_to = "value") %>%
  group_by(params) %>% summarize(average=mean(value))
