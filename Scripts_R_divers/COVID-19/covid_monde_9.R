...
download.file("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv", destfile = "data/covid19_confirmed_06mai2021.csv")
download.file("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv", destfile = "data/covid19_deaths_06mai2021.csv")
download.file("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_recovered_global.csv", destfile = "data/covid19_recovered_06mai2021.csv")

download.file("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/UID_ISO_FIPS_LookUp_Table.csv", destfile = "data/UID_ISO_FIPS_LookUp_Table_06mai2021.csv")

# Charger les identifiants
covid_uid <- read_csv("data/UID_ISO_FIPS_LookUp_Table_06mai2021.csv") %>%
  ...
  filter(!Country.Region %in% c("Palau")) %>%
  arrange(Country.Region)

# Confirmés
covid_confirmed <- read_csv("data/covid19_confirmed_06mai2021.csv") %>%
...
# filtrer pays avec - de 20 cas confirmés au 30 avril 2021 (colonne 470)
covid_confirmed <- covid_confirmed %>% filter_at(470, all_vars(. >= 20))
...
covid_confirmed_long <- filter(covid_confirmed_long, date < "2021-05-01")
...
covid_confirmed_long <- filter(covid_confirmed_long, !Country.Region %in% c("Australia", "Canada", "China", "MS Zaandam", "Diamond Princess"))
covid_confirmed_long <- filter(covid_confirmed_long, Country != "Bonaire, Sint Eustatius and Saba, Netherlands", "Marshall Islands", "Micronesia", "Saint Helena", "Samoa", "Vanuatu")


covid_confirmed_long <- bind_rows(covid_confirmed_long, australia, canada, china) %>% arrange(Country)

length(unique(covid_confirmed_long$Country)) # doit faire 213


# Décédés
covid_deaths <- read_csv("data/covid19_deaths_06mai2021.csv") %>%
  rename("Province.State" = "Province/State") %>% rename("Country.Region" = "Country/Region") %>%
  filter(!Country.Region %in% c("Marshall Islands", "Micronesia", "Saint Helena", "Samoa", "Vanuatu"))
...
covid_deaths_long <- filter(covid_deaths_long, date < "2021-05-01")
...
length(unique(covid_deaths_long$Country)) # doit faire 213

# Guéris

covid_recovered <- read_csv("data/covid19_recovered_06mai2021.csv") %>%
  rename("Province.State" = "Province/State") %>% rename("Country.Region" = "Country/Region") %>%
  filter(!Country.Region %in% c("Marshall Islands", "Micronesia", "Saint Helena", "Samoa", "Vanuatu"))
...
covid_recovered_long <- filter(covid_recovered_long, date < "2021-05-01")
...
length(unique(covid_recovered_long$Country)) # doit faire 213
...
jhu.covid.3 <- jhu.covid.3 %>% arrange(Country.Region)
...
jhu.covid.3 <- jhu.covid.3 %>% arrange(Country)
...
dat.last <- dplyr::filter(jhu.covid.3, date == last_date) %>%
  rename(confirmed_last = confirmed, deaths_last = deaths, recovered_last = recovered) %>%
  dplyr::select(-date) %>%
  arrange(ISO_A3)
...
# download.file("https://open-covid-19.github.io/data/data_minimal.csv", destfile="data/covid_ecdc_06mai2021.csv")
# 
# covid_fr_ecdc <- read_csv("data/covid_ecdc_06mai2021.csv")
# covid_fr_ecdc <- filter(covid_fr_ecdc, Key == "FR" & Date > "2020-01-21" & Date < "2021-05-01")
# 
...
# 
# write_excel_csv(jhu.covid.3, "JHU_covid19_data_06mai2021.csv")
...
worldmap <- ggplot(World) +
  ...
  geom_point(data=dat.last, aes(x=Long, y=Lat, size=confirmed_last, colour=deaths_last, stroke=0), alpha=0.6) +
...
write_excel_csv(covid_result_table, "covid_result_table_06mai2021.csv")
...
World2 <- World2 %>% filter(ISO_A3 != "ATF", GEOUNIT != "Aland", GEOUNIT != "Cook Islands", GEOUNIT != "Guam", GEOUNIT != "Guernsey", GEOUNIT != "Hong Kong S.A.R.", GEOUNIT != "Heard Island and McDonald Islands", GEOUNIT != "British Indian Ocean Territory", GEOUNIT != "Jersey", GEOUNIT != "Kiribati", GEOUNIT != "Macao S.A.R", GEOUNIT != "Marshall Islands", GEOUNIT != "Northern Mariana Islands", GEOUNIT != "Norfolk Island", GEOUNIT != "Niue", GEOUNIT != "Nauru", GEOUNIT != "Pitcairn Islands", GEOUNIT != "Palau", GEOUNIT != "Puerto Rico", GEOUNIT != "North Korea", GEOUNIT != "South Georgia and the Islands", GEOUNIT != "Turkmenistan", GEOUNIT != "Tuvalu", GEOUNIT != "United States Minor Outlying Islands", GEOUNIT != "United States Virgin Islands", GEOUNIT != "Tonga")
