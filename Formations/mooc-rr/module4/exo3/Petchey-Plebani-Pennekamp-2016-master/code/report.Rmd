...
## change the Day variable to character, to help with the merge, and back after
tr$Day.number <- as.character(tr$Day.number)
scaled_trimmed_final$Day.number <- as.character(scaled_trimmed_final$Day.number)
two.datasets <- inner_join(tr, scaled_trimmed_final)
two.datasets$Day.number <- as.numeric(two.datasets$Day.number)

## some graphs that are of use for very detailed views of the two datasets. Not included in report by default for brevity.
# ggplot(filter(two.datasets), aes(x=as.numeric(Day.number), y=stand.y)) +
#   geom_path() +
#   facet_wrap(~variable) +
#   geom_line(aes(y=Abundance), col="red")

# ggplot(filter(two.datasets, variable=="Soluble.reactive.phosphorus"), aes(x=as.numeric(Day.number), y=stand.y)) +
#   geom_path() +
#   facet_wrap(~variable) +
#   geom_line(aes(y=Abundance), col="red")
...
## shorten the names of the nutrients
two.datasets$variable <- plyr::revalue(two.datasets$variable,
                                 c(Soluble.reactive.phosphorus="P",
                                   Total.dissolved.inorganic.nitrogen="N"))

# ensure alphabetical order of panels in figure
two.datasets$variable <- factor(two.datasets$variable,
                                 levels = sort(levels(as.factor(two.datasets$variable))))
...
ggplot(two.datasets, aes(x=Abundance, y=stand.y)) + geom_point() +
  ...
  xlab("Values from published data") +
...
```{r, eval=F, echo=F}
## code to export the interpolated data, with which to test the GAM functions
mt_wide <- spread(mt, key=variable, value=value)
write.csv(mt_wide, "~/Desktop/Repro_data_for_gam_test.csv")
```
...
```{r, eval=F, echo=F}
## code to export the interpolated and transformeddata, with which to test the GAM functions
write.csv(X, "~/Desktop/Repro_data_for_gam_test_transformed.csv")
```
...
## Some preliminaries, including arranging storage
T_lags <- 12
num_spp <- length(gams)
rsq_vals <- matrix(NA, T_lags, num_spp)
dimnames(rsq_vals)[[2]] <- names(gams)

## fit gams for all the required lags
for(i in 1:T_lags) {
  gams <- Fit_GAMs(X, gval=1.4, T_lag=i)
  ## and get the r-squared for each
  for(j in 1:num_spp)
    rsq_vals[i,j] <- cor(gams[[j]]$fitted.values, gams[[j]]$model[1])^2
}

#save(rsq_vals, file="~/Desktop/rsq_vals.Rdata")
