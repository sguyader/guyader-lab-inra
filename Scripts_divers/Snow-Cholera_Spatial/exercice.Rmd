...
# Téléchargeons la carte à partir du serveur Stamen, et utilisons le type "toner-lite" pour une visualisation plus aisée
js_map <- get_stamenmap(bbox = bbox, zoom = 16, maptype = "toner-lite", crop = TRUE, messaging = FALSE,  urlonly = F, color = "color", force = FALSE,  where = tempdir())
...
snow_pp <- ppp(coordinates(snow_df)[,1], coordinates(snow_df)[,2], window = win, marks = snow_df$Id)
...
# graphique superposant la densité du nombre de décès, la position des pompes, et les tessels :
## densité des décès
plot(density(split(snow_pp)$d, sigma=2.5e-04), col=colorRampPalette(c("white","yellow","red")),
     main="", show.all=F)
## position des pompes
...
```{r}
# estimation par Khi2 de la répoartition aléatoire ou non du nombre de décès dans les tessels
quadr.test <- quadrat.test(split(snow_pp)$d, tess=tessels)

# graph superposant la densité du nombre de décès, la position des pompes, et les tessels :
# densité des décès
plot(density(split(snow_pp)$d, sigma=2.5e-04), col=colorRampPalette(c("white","yellow","red")),
     main="", show.all=F)
# position des pompes
...
js_fond +
  ...
  geom_point(data=data.frame(pompes_df),
             aes(x=coords.x1, y=coords.x2), shape=9, col="blue", size=2, alpha=0.8) +
  # ajout des tessels
  geom_polygon(data=tessels_df,
               aes(x = long, y = lat, group = group), color = "blue", fill = NA, size=0.5) +
